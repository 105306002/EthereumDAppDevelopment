...
// TODO
//鑄 NCCU Coin
let mintCoinVal = $('#mintCoinVal');
let mintButton = $('#mintButton');
//購買 NCCU Coin
let buyCoinVal = $('#buyCoinVal');
let buyButton = $('#buyButton');
//轉帳 NCCU Coin
let transferCoinTo = $('#transferCoinTo');
let transferCoinVal = $('#transferCoinVal');
  ...
  if (bankAddress != '') {
    $.get(
      ...
      function (result) {
        ...
        $('#coinBalance').text('銀行Coin餘額 (wei): ' + result.coinBalance);
      }
    );
  } else {
    $.get(
...
mintButton.on('click', async function () {
  ...
  $.post(
    '/mintCoin', {
      address: bankAddress,
      account: nowAccount,
      value: parseInt(mintCoinVal.val(), 10)
    },
    function (result) {
      if (result.events !== undefined) {
        log(result.events.MintEvent.returnValues, '鑄 NCCU Coin成功');
...
buyButton.on('click', async function () {
  ...
  $.post(
    '/buyCoin', {
      address: bankAddress,
      account: nowAccount,
      value: parseInt(buyCoinVal.val(), 10)
    },
    function (result) {
      if (result.events !== undefined) {
...
transferCoinButton.on('click', async function () {
  ...
  $.post(
    '/transferCoin', {
      ...
      value: parseInt(transferCoinVal.val(), 10)
    },
    function (result) {
      if (result.events !== undefined) {
